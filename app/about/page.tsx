...
export default function AboutPage() {
  return <div className={styles.page} id="top">
    ...
    <main id="main-content">
      ...
      <section className={styles.directors} id="directors" aria-labelledby="directors-title"><div className={styles.container}><header><p className={styles.eyebrow}>OUR DIRECTORS</p><h2 id="directors-title">The people helping shape<br /><em>our direction.</em></h2></header><div className={styles.directorGrid}>{directors.map((director, index) => <article key={director.id}><div className={styles.directorPhoto}><Image src={director.image} alt={director.imageAlt} fill sizes="(max-width: 760px) 86vw, 36vw" style={{ objectPosition: director.imagePosition }} /></div><div className={styles.directorInfo}><span>0{index + 1} / DIRECTOR</span><h3>{director.name}</h3><p className={styles.directorFocus}>{director.focus}</p><p>{director.paragraphs[0]}</p><ul>{director.expertise.map(item => <li key={item}>{item}</li>)}</ul></div></article>)}</div></div></section>
      ...
      <section className={styles.cta} aria-labelledby="cta-title"><div className={styles.container}><p className={styles.eyebrow}>LET’S BUILD WHAT COMES NEXT</p><h2 id="cta-title">Bring us the challenge.<br /><em>We’ll build the intelligent way forward.</em></h2><div><a className={styles.primary} href={strategyUrl} target="_blank" rel="noopener noreferrer">Book a strategy session <span aria-hidden="true">↗</span></a><Link className={styles.secondary} href="/contact">Contact our team</Link></div></div></section>
    </main>
