...
export default function ProductsPage() {
  return <div className={styles.page} id="top">
    ...
    <main id="main-content">
      ...
      <section className={styles.indiaSection} id="product-integrations" aria-labelledby="india-title"><div className={`${styles.container} ${styles.indiaLayout}`}>
        <div><p className={styles.eyebrow}>BUILT HERE. FOR THE WAY YOU WORK.</p><h2 id="india-title">Indian workflows.<br /><em>Understood.</em></h2><p>Built around the way Indian businesses actually operate—from everyday communication and accounting to approval processes and multilingual customer journeys.</p><span className={styles.indiaAudience}>For SMEs, professional firms and growing enterprises.</span></div>
        <div className={styles.integrationPanel}><div className={styles.integrationTags}>{['WhatsApp', 'Tally', 'CRM & ERP', 'Spreadsheets', 'Calendars', 'Internal tools'].map(tool => <span key={tool}>{tool}</span>)}</div><ul><li>WhatsApp-first communication and reminders</li><li>Tally-compatible analytics and data workflows</li><li>English, Hindi, Hinglish and selected Indian-language experiences</li><li>Flexible integrations with the systems you use</li><li>Your approval, reporting and escalation logic</li></ul></div>
      </div></section>
      ...
      <ProductJourney />
      <ProductFAQ />
      <section className={styles.finalCta} id="contact" aria-labelledby="contact-title"><div className={styles.container}><p className={styles.eyebrow}>YOUR NEXT CHAPTER STARTS HERE</p><h2 id="contact-title">Choose the right AI product<br /><em>for your business.</em></h2><p>Tell us where your business is losing time, leads, visibility or revenue. Our team will help you identify the most suitable Bizgenix product and implementation path.</p><div className={styles.actions}><a className={styles.primary} href={demoUrl()} target="_blank" rel="noopener noreferrer">Book a Product Demo <span aria-hidden="true">↗</span></a><a className={styles.secondary} href="tel:+918200858674">Talk to an AI Expert</a><a className={styles.exploreLink} href="#product-suite">Explore Products</a></div></div></section>
