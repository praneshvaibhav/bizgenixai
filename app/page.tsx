...
export default function Home(){const[menu,setMenu]=useState(false);return <main><ScrollAnimations/>
<nav className="nav"><a href="#top"><img src="/bizgenixlogo.webp" alt="Bizgenix AI" width={640} height={233} fetchPriority="high"/></a><button className="menu" onClick={()=>setMenu(!menu)} aria-label="Toggle navigation">☰</button><div className={menu?'links open':'links'}>{[['Home','#top'],['About','#about'],['Custom Solution','/custom-solutions'],['Product','#products'],['Programs','#programs']].map(([label,href])=><a key={label} onClick={()=>setMenu(false)} href={href}>{label}</a>)}</div></nav>
